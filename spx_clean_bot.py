...
pass
def send_weekend_notification(entering):
t = now_riyadh()
...
print("[BLACKOUT] message suppressed")
return
...
best_txt = bt.direction + " | " + bt.result_label + " | " + "{:+.2f}".format(bt.r_res
if summary["worst_trade"]:
wt = summary["worst_trade"]
worst_txt = wt.direction + " | " + wt.result_label + " | " + "{:+.2f}".format(wt.r_re
return (
CFG.user_title + " - Daily Stats\n"
"Date: " + target_date.isoformat() + " (Riyadh)\n\n"
"Total: " + str(summary["total"]) + "\n"
"Wins: " + str(summary["wins"]) + " | Losses: " + str(summary["losses"]) + " | "Win Rate: " + "{:.1f}".format(summary["winrate"]) + "%\n"
"Net: " + "{:+.2f}".format(summary["net_r"]) + "R\n\n"
"T1: " + str(summary["t1_only"]) + " | T2: " + str(summary["t2_hit"]) + " | T3: " + s
"Range: " + str(summary["range_count"]) + " | Trending: " + str(summary["trending_cou
"Messy: " + str(summary["messy_count"]) + " | Weak: " + str(summary["weak_count"]) +
"Best: " + best_txt + "\n"
"Worst: " + worst_txt
BE: "
)
def weekly_stats_message(summary, monday, friday):
best_txt = "N/A"
worst_txt = "N/A"
if summary["best_trade"]:
bt = summary["best_trade"]
...
worst_txt = wt.direction + " | " + wt.result_label + " | " + "{:+.2f}".format(wt.r_re
return (
CFG.user_title + " - Weekly Stats\n"
"Period: " + monday.isoformat() + " to " + friday.isoformat() + "\n\n"
"Total: " + str(summary["total"]) + "\n"
"Wins: " + str(summary["wins"]) + " | Losses: " + str(summary["losses"]) + " | "Win Rate: " + "{:.1f}".format(summary["winrate"]) + "%\n"
"Net: " + "{:+.2f}".format(summary["net_r"]) + "R\n\n"
"T1: " + str(summary["t1_only"]) + " | T2: " + str(summary["t2_hit"]) + " | T3: " + s
...
global TV
TV = make_tv_client()
print("[INFO] TradingView client reinitialized")
def _tv_get_hist(symbol, exchange, interval, n_bars):
global TV, LAST_GOOD_DATA
cache_key = exchange + ":" + symbol + ":" + str(interval)
...
print("[WARN] Using cached data for " + cache_key)
return cached.copy()
raise RuntimeError("TV fetch failed: " + exchange + ":" + symbol + " | " + repr(last_err)
...
return exchange + ":" + symbol, df_4h, df_1h, df_15m, df_5m
# =========================
# Structure / Levels
# =========================
...
return "Range"
return "Weak"
def cluster_levels(levels, tol_frac, price_ref):
if not levels:
return []
...
return ", ".join(["{:.1f}".format(float(x)) for x in levels])
def near_level(price, level, tol_frac):
return abs(price - level) / max(price, 1e-9) <= tol_frac
def nearest_opposite_level_distance(levels, level_hit, direction):
if not levels:
return None
if direction == "BUY":
...
def nearby_levels_count(levels, level_hit, within_pts):
...
return False, "range level rejected: too little space (" + "{:.1f}".format(space_pts)
...
return False, "range level rejected: congestion (" + str(nearby) + " nearby)"
...
def classify_target_zone(target_idx, prob):
if target_idx == 1:
...
return out
def filter_target_map_for_message(target_map):
if not target_map:
return []
kept = []
...
# Indicators
# =========================
def compute_indicators_5m(df_5m):
out = df_5m.copy()
...
macd = MACD(close=out["close"], window_slow=26, window_fast=12, window_sign=
out["macd_hist"] = macd.macd_diff()
return out
def stoch_cross(df_5m, direction):
...
return False
def momentum_shift(df_5m, direction):
if len(df_5m) < 30:
return False
c = float(df_5m["close"].iloc[-1])
ema = float(df_5m["ema20"].iloc[-1])
...
return (c < ema) or (hist_prev > 0 and hist < 0)
return False
def strong_buy_confirmation(df_5m):
if len(df_5m) < 5:
return False
c = float(df_5m["close"].iloc[-1])
ema = float(df_5m["ema20"].iloc[-1])
...
def strong_sell_confirmation(df_5m):
if len(df_5m) < 5:
return False
c = float(df_5m["close"].iloc[-1])
ema = float(df_5m["ema20"].iloc[-1])
...
d = float(df_5m["stochrsi_d"].iloc[-1])
return (c < ema) and (macd_hist < 0) and (k <= d)
def rejection_lite(df_5m, direction):
c = df_5m.iloc[-1]
o = float(c["open"])
h = float(c["high"])
l = float(c["low"])
...
rng = max(h - l, 1e-9)
if direction == "BUY":
return (min(o, cl) - l) / rng >= 0.25
if direction == "SELL":
return (h - max(o, cl)) / rng >= 0.25
return False
def break_retest(df_5m, level, direction):
...
return bool(broke and retest and last_close < level)
return False
def momentum_breakout(df_5m, level, direction):
...
if CFG.momentum_breakout_needs_ema:
cond = cond and closes[2] > ema
if CFG.momentum_breakout_needs_macd:
cond = cond and macd_hist > 0
return bool(cond)
if direction == "SELL":
...
def compute_market_state(df_1h, df_4h, prev_label):
...
if dist < 0.1:
return (1, 5)
mult_map = {"Low": 1.35, "Normal": 1.05, "High": 0.90, "Extreme": 0.80}
mult = mult_map.get(liq_state, 1.0)
eta = (dist / vel) * mult
...
# Wick / Level quality
# =========================
def wick_cluster_near_level(df_5m, level):
...
def count_level_touches(df_5m, level, lookback=40, abs_tol=2.0):
...
touches += 1
return touches
def level_quality_info(df_5m, level_now, level):
...
def choose_best_level(df_5m, level_now, key_levels):
ranked = []
...
# =========================
def classify_trade_strength(score, conf):
...
return t1, t2, t3
def pick_targets(levels, entry, direction):
if not levels:
return None, None
if direction == "BUY":
...
def compute_range_adaptive_extra_stop(market_label, exp_move_val):
if market_label != "Range":
return 0.0
...
if direction == "BUY" and (t2 is None or atr_t3 > t2):
t3 = atr_t3
if direction == "SELL" and (t2 is None or atr_t3 < t2):
t3 = atr_t3
...
# =========================
def confidence_percent(score, direction, bias, session, market_label, liq_state):
...
def probability_t1_t2(conf, rr, dist_t1, dist_t2, exp_move_1h_val, liq_state, market_label):
t1 = float(conf)
...
# Messages
# =========================
def format_target_map_for_message(target_map):
if not target_map:
return "N/A"
lines = []
for t in target_map:
lines.append(t["name"] + ": " + safe_f1(t["price"]) + " | " + str(safe_int(t["prob"])
return "\n".join(lines)
def signal_message(session, symbol, market_state_str, liq_state,
level_now, direction, level_hit, plan,
score, conf, p_t1, p_t2, exp_move, eta_band, reasons):
if exp_move and np.isfinite(float(exp_move)):
exp_txt = "+-" + "{:.0f}".format(float(exp_move)) + " pts"
else:
exp_txt = "N/A"
eta_txt = "N/A" if eta_band is None else str(eta_band[0]) + "-" + str(eta_band[1]) target_map_txt = format_target_map_for_message(plan.get("target_map", []))
ts = now_riyadh().strftime("%Y-%m-%d %H:%M")
+ " mi
return (
CFG.user_title + " - Entry Signal (Hunter Smart)\n\n"
"Time: " + ts + " (Riyadh)\n"
"Session: " + session + "\n"
"Symbol: " + symbol + "\n\n"
"Market: " + market_state_str + "\n"
"Liquidity: " + liq_state + "\n"
"Price: " + safe_f1(level_now) + "\n\n"
"Direction: " + direction + "\n"
"Level: " + safe_f1(level_hit) + "\n"
"Trade Type: " + plan.get("trade_type", "N/A") + "\n\n"
"Entry: " + safe_f1(plan.get("entry")) + "\n"
"Stop: " + safe_f1(plan.get("stop")) + "\n"
"T1: " + safe_f1(plan.get("t1")) + "\n"
"T2: " + safe_f1(plan.get("t2")) + "\n"
"T3: " + safe_f1(plan.get("t3")) + "\n\n"
"RR: " + safe_f2(plan.get("rr")) + "\n"
"Score: " + str(score) + "/6\n"
"Confidence: " + str(conf) + "%\n\n"
"Probability\n"
"T1: " + str(p_t1) + "%\n"
...
"Target Map\n"
+ target_map_txt + "\n\n"
"Expected Move (1H): " + exp_txt + "\n"
"ETA to T1: " + eta_txt + "\n\n"
"Reason: " + ", ".join(reasons)
)
def hourly_update_message(session, symbol, bias, market_state_str,
levels, price, active_trade):
status = "None" if active_trade is None else active_trade.get("status", "Unknown")
trade_line = "-"
target_map_line = "N/A"
if active_trade is not None:
trade_line = (
active_trade.get("direction", "?") + " | "
"Entry " + safe_f1(active_trade.get("entry")) + " | "
"Stop " + safe_f1(active_trade.get("stop")) + " | "
"T1 " + safe_f1(active_trade.get("t1")) + " | "
"T2 " + safe_f1(active_trade.get("t2")) + " | "
"T3 " + safe_f1(active_trade.get("t3"))
)
target_map_line = format_target_map_for_message(active_trade.get("target_map", ts = now_riyadh().strftime("%Y-%m-%d %H:%M")
return (
CFG.user_title + "\n"
"Hourly Update (" + ts + " Riyadh)\n"
"Session: " + session + "\n"
"Symbol: " + symbol + "\n"
"Bias (1H/4H): " + bias + "\n"
"Market: " + market_state_str + "\n"
"Price: " + safe_f1(price) + "\n"
"Key Levels: " + fmt_levels(levels) + "\n"
"Active Trade: " + status + "\n"
"Trade: " + trade_line + "\n"
"Targets:\n" + target_map_line
)
...
# === PART 3 ===
# BotState, Reset, Wick Validation, Scoring, Dynamic Extension, Trade Recording, Update Activ
# =========================
# Bot State
# =========================
class BotState:
...
send_telegram(CFG.user_title + " - Daily Reset\nTrade continues - new day started.")
else:
send_telegram(CFG.user_title + " - Daily Reset\nNo active trade - new day started.")
...
def maybe_update_market_state(df_1h, df_4h):
nowu = datetime.utcnow()
should = (
...
market_label, market_dir, df_5m, score):
wick_info = level_info["wick_info"]
...
wick_reason = "Upper-wick rejection cluster near " + wick_info.get("bucket", "N/A")
if wick_info.get("lower_cluster") and direction == "BUY":
wick_reason = "Lower-wick cluster near support " + wick_info.get("bucket", "N/A")
if wick_reason:
score += 3
...
or strong_buy_confirmation(df_5m) if direction == "BUY" else strong_sell_confirmation
)
...
# =========================
# Dynamic extension - single definition
# =========================
def next_levels_beyond(levels, price_ref, direction):
if direction == "BUY":
return sorted([lvl for lvl in levels if lvl > price_ref])
return sorted([lvl for lvl in levels if lvl < price_ref], reverse=True)
def dynamic_extension_allowed(df_5m, direction):
if direction == "BUY":
...
market_label, market_dir, liq_state):
...
friday = monday + timedelta(days=4)
week_key = monday.isoformat() + "_" + friday.isoformat()
...
if stop_hit:
send_telegram(
CFG.user_title + " - Pending Trade Cancelled\n"
"Direction: " + direction + " | Entry: " + safe_f1(entry) + "\n"
"Price reached stop " + safe_f1(stop) + " before activation.\n"
"Auto-cancelled."
)
...
if idx == 1:
send_telegram(
CFG.user_title + " - " + name + " Hit\n"
+ name + ": " + safe_f1(tgt_price) + "\n"
"Probability was: " + str(safe_int(info.get("prob"))) + "%\n"
"Stop remains: " + safe_f1(tr["stop"])
)
else:
msg = (
...
if weak:
msg += "\nWeakening Watch: " + weak_reason
send_telegram(msg)
# SPXW tracking on target hit
try:
spxw_on_target_hit(
target_name=name,
...
print("[WARN] spxw_on_target_hit: " + repr(e))
# Extend next target
...
send_telegram(
CFG.user_title + " - Reversal Watch Zone\n"
"Last Hit: T" + str(max_hit_idx) + "\n"
"Reason: " + weak_reason + "\n"
"Note: monitoring weakness - not a reversal trade."
)
...
print("[WARN] spxw_on_stop_hit: " + repr(e))
STATE.active_trade = None
return
...
send_telegram(
CFG.user_title + " - Stop Test\n"
"Price: " + safe_f1(last_price) + " | Stop: " + safe_f1(stop) + "\n"
"Waiting for 5m candle close confirmation."
)
else:
