# -*- coding: utf-8 -*-
"""
ES Trading Bot (Hunter Smart v6 - Reaction Zones + Dynamic Targets)
For: دكتور محمد

Core features:
- Symbol ALWAYS: ES=F
- ES Adjust: -10 points
- Reaction Zones instead of rigid S/R naming
...
- Dynamic targets:
    * Weak -> micro
    * Standard -> zone + ATR assist
    * Strong -> extended ATR/zone targets
- Trade upgrade:
    * live trade can expand targets if setup strengthens
- Keeps:
    * Confidence
    * RR
    * Probability
...
class Config:
    ...
    max_zone_distance_pts: float = 85.0
    ...
    # Dynamic targets / upgrade
    strong_score_threshold: int = 5
    strong_conf_threshold: int = 60
    standard_conf_threshold: int = 50
    enable_trade_upgrade: bool = True
    upgrade_check_min_minutes: int = 5
    upgrade_target_atr_mult_t2: float = 0.8
    upgrade_target_atr_mult_t3: float = 1.2

    # T2 cap
    cap_t2_enabled: bool = True
    cap_factor_market: float = 1.35
...
def compute_trade_plan(
    ...
    base_t1, base_t2 = pick_targets_from_zones(entry, direction, zones)
    t1, t2, t3 = base_t1, base_t2, None

    if exp_move_val is not None and np.isfinite(exp_move_val):
        if trade_strength == "Weak":
            pass
        elif trade_strength == "Standard":
            atr_t2 = entry + (exp_move_val * 0.55) if direction == "BUY" else entry - (exp_move_val * 0.55)
            if t2 is None:
                t2 = float(atr_t2)
        elif trade_strength == "Strong":
            atr_t2 = entry + (exp_move_val * 0.75) if direction == "BUY" else entry - (exp_move_val * 0.75)
            atr_t3 = entry + (exp_move_val * 1.15) if direction == "BUY" else entry - (exp_move_val * 1.15)

            if t2 is None:
                t2 = float(atr_t2)
            else:
                if direction == "BUY":
                    t2 = max(float(t2), float(atr_t2))
                else:
                    t2 = min(float(t2), float(atr_t2))
    ...
    # cap T2 if needed
    if CFG.cap_t2_enabled and t2 is not None and exp_move_val is not None and np.isfinite(exp_move_val):
        factor = CFG.cap_factor_market if session == "Market" else CFG.cap_factor_offhours
        cap_dist = exp_move_val * factor
...
def signal_message(
    ...
    trade_type = plan.get("trade_strength", "N/A")
...
def maybe_upgrade_active_trade(df_5m: pd.DataFrame, exp_move_val: float | None):
    if not CFG.enable_trade_upgrade:
        return
    ...
    live_since = tr.get("live_since_utc")
    if live_since is None:
        return

    mins_live = (datetime.utcnow() - live_since).total_seconds() / 60.0
    if mins_live < CFG.upgrade_check_min_minutes:
        return
    ...
    strong_follow = False
    if direction == "BUY":
        strong_follow = price_now > ema and ms
    else:
        strong_follow = price_now < ema and ms

    if not (strong_follow or st):
        return
    ...
    if direction == "BUY":
        new_t2 = entry + (exp_move_val * CFG.upgrade_target_atr_mult_t2)
        new_t3 = entry + (exp_move_val * CFG.upgrade_target_atr_mult_t3)
        if tr.get("t2") is None or new_t2 > float(tr["t2"]):
            tr["t2"] = float(new_t2)
        tr["t3"] = float(new_t3)
    else:
        new_t2 = entry - (exp_move_val * CFG.upgrade_target_atr_mult_t2)
    ...
    tr["upgraded"] = True
    tr["trade_strength"] = "Strong"

    send_telegram(
        f"🚀 {CFG.user_title} — Trade Upgraded\n"
        f"Direction: {direction}\n"
        f"New T2: {safe_f1(tr.get('t2'))}\n"
        f"New T3: {safe_f1(tr.get('t3'))}\n"
        f"Reason: momentum continuation / stronger setup"
    )
...
def update_active_trade(df_5m: pd.DataFrame, last_price: float, exp_move_val: float | None):
    tr = STATE.active_trade
    if tr is None:
        return
    ...
    maybe_upgrade_active_trade(df_5m, exp_move_val)
...
def evaluate_once():
    ...
    if STATE.active_trade is not None:
        update_active_trade(df_5m, price_now, exp_move_val)
    ...
    # role by behavior, not fixed label
    ...
    trade_strength = classify_trade_strength(score, conf)
    ...
    key = f"{direction}:{safe_f1(zone_mid(acting_zone))}:{trigger}:{trade_strength}"
...
def main():
    send_telegram(
        ...
        f"- Dynamic Targets: Weak / Standard / Strong\n"
        f"- Live Upgrade: ON\n"
